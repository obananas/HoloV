...
from .utils import adjust_ids,HoloV,scaled_dot_product_attention
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        if position_embeddings is None:
            logger.warning_once(
                "The attention layers in this model are transitioning from computing the RoPE embeddings internally "
                "through [rotary_pos_emb](file://d:\code\Qwen2.5-VL\model\modeling_qwen2_5_vl.py#L0-L0) (2D tensor of RoPE theta values), to using externally computed "
                "[position_embeddings](file://d:\code\LLaVA-NeXT\llava\model\multimodal_resampler\qformer.py#L0-L0) (Tuple of tensors, containing cos and sin). In v4.54 [rotary_pos_emb](file://d:\code\Qwen2.5-VL\model\modeling_qwen2_5_vl.py#L0-L0) will be "
                "removed and [position_embeddings](file://d:\code\LLaVA-NeXT\llava\model\multimodal_resampler\qformer.py#L0-L0) will be mandatory."
            )
            emb = torch.cat((rotary_pos_emb, rotary_pos_emb), dim=-1)
            cos = emb.cos()
            sin = emb.sin()
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, output_attentions: bool = True) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
        Args:
            ...
            output_attentions (`bool`, *optional*):
                Whether or not to return the attention weights of the last layer.

        Returns:
            `torch.Tensor`: hidden_states and optionally attention weights from the last layer.
        """
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states, attn_weights = blk(
                    ...
                    position_embeddings=position_embeddings,
                    output_attentions=output_last_layer_attention
                )
        ...
        if last_attention_weights is not None:
            ...
            if last_attention_weights.dim() == 4:  # [1, num_heads, seq_len, seq_len]
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_token_start = torch.where(input_ids[0]==self.config.vision_start_token_id)[0]
                image_token_len = image_embeds.shape[0]
                text_tokens_start = torch.where(input_ids[0]==self.config.vision_end_token_id)[0]
...
######################################change###########################################
        ...
        if pixel_values is not None:
            ...
            if prune_method == "HoloV":
                ...
                raise ValueError(f"Unknown prune_method: {prune_method}. Must be 'merge_tokens' or 'token_pruning'")
